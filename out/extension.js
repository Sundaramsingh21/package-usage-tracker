...
const DEPENDENCY_FIELDS = ['dependencies'];
...
function activate(context) {
    ...
    const watcher = vscode.workspace.createFileSystemWatcher(`**/*.{${ALL_SCAN_EXTENSIONS.join(',')}}`);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('pkgUsage.seeMore', async (packageName) => {
        await showSeeMorePopup(packageName);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('pkgUsage.searchWorkspace', async (packageName) => {
        await vscode.commands.executeCommand('workbench.action.findInFiles', {
            query: packageName,
            triggerSearch: true
        });
    }));
    context.subscriptions.push(vscode.commands.registerCommand('pkgUsage.refresh', async () => {
        fileCache.clear();
        packageIndex.clear();
        filesListedAt = 0;
        refreshActiveEditor();
...
async function handleFileChanged(uri) {
...
function removeFromCache(fsPath) {
...
async function updateDecorations(editor) {
    ...
    for (const pkg of packages) {
        ...
        const range = new vscode.Range(new vscode.Position(lineIndex, lineLength), new vscode.Position(lineIndex, lineLength));
        ...
        if (usage.totalCount > 0) {
            ...
            hover.appendMarkdown(`**${pkg}** is used **${usage.totalCount}** time(s) across **${usage.files.length}** file(s).\n\n`);
            ...
            hover.appendMarkdown(`**${pkg}** doesn't appear to be used in any scanned source or style file.\n\n`);
...
function findDependencyLine(lines, pkgName) {
...
async function ensureWorkspaceScanned() {
    ...
    const now = Date.now();
    if (workspaceFiles.length === 0 || now - filesListedAt > 30000) {
        workspaceFiles = await vscode.workspace.findFiles(`**/*.{${ALL_SCAN_EXTENSIONS.join(',')}}`, EXCLUDE_GLOB);
        filesListedAt = now;
    }
    const stale = [];
...
async function parseAndCacheFile(uri) {
    let mtime;
    let content;
    try {
        const stat = await vscode.workspace.fs.stat(uri);
...
function parseScriptFileForPackages(content) {
    ...
    const noteFirstIndex = (entry, index) => {
...
function parseStyleFileForPackages(content) {
    const bindings = new Map();
    const ensure = (pkg) => {
        let entry = bindings.get(pkg);
        if (!entry) {
...
function finalizeBindings(bindings, content) {
...
function addIdentifiersFromMatch(group, into) {
    if (!group)
        return;
...
function getUsageForPackage(pkgName) {
    const result = { totalCount: 0, files: [] };
...
async function showSeeMorePopup(pkgName) {
